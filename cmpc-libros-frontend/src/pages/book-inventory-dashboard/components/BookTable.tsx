...
interface BookTableProps {
  readonly books: ReadonlyArray<BookRow>;
  readonly loading: boolean;
  readonly selectedBooks: ReadonlyArray<string>;
  readonly onSelectionChange: (ids: ReadonlyArray<string>) => void;
  readonly onSort: (field: keyof BookRow | string) => void;
              ...
              <tr key={book.id} className="hover:bg-muted/50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <Checkbox
                    ...
                    onChange={(e) => handleSelectBook(book.id, Boolean((e.target as HTMLInputElement).checked))}
                  />
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{book.author}</td>
                
                <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{book.publisher}</td>
                
                <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{book.genre}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${book.available ? 'bg-green-100 text-green-800' :'bg-red-100 text-red-800'}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex items-center justify-end space-x-2">
                    <Button variant="ghost" size="sm" onClick={() => onView(book.id)} iconName="Eye" className="text-blue-600 hover:text-blue-700" />
                    <Button variant="ghost" size="sm" onClick={() => onEdit(book.id)} iconName="Edit" className="text-green-600 hover:text-green-700" />
                    <Button variant="ghost" size="sm" onClick={() => onDelete(book.id)} iconName="Trash2" className="text-red-600 hover:text-red-700" />
                  </div>
                </td>
              </tr>
            ))}
